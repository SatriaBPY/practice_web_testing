...
export async function globalSetup(config: FullConfig) {
  ...
  const dirPath = path.join(process.cwd(), "auth/state");
  fs.mkdirSync(dirPath, { recursive: true });

  const storageFile = path.join(dirPath, `storageState.json`);

  await page.context().storageState({ path: storageFile });

  await browser.close();
  console.log(
    `[${new Date().toLocaleTimeString()}] ✅ AUTH: Login successful for valid user.`,
  );
  console.log(`--------------------------------------\n`);
}
